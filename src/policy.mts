...
export interface Policy {
  ...
  allowedPaths: string[];
  testPatterns: string[];
  criteria: { id: string; behavior: string; failures: string[] }[];
  checks: Check[];
  testChanges: {
    path: string;
  ...
  environment: { target: string; seed: string };
  review: { threadId: string; model: string; reasoning: string } | null;
}
...
function texts(value: unknown, nonempty = true): asserts value is string[] {
  requireThat(Array.isArray(value) && (!nonempty || value.length > 0), "Expected a string array");
  value.forEach(text);
}
export function safePath(value: string, pattern = false): void {
  requireThat(
    value.length > 0 &&
      !value.startsWith("/") &&
      !value.includes("\\") &&
