...
PROFILE_KIND = "proofloop-sol-luna/project-profile"
CONTRACT_KIND = "proofloop-sol-luna/run-contract"
ADMISSION_KIND = "proofloop-sol-luna/test-admission"
LUNA_MODES = {"bounded-verifier"}
...
def _profile(document: dict[str, Any], errors: list[str]) -> None:
    required = {"schema_version", "kind", "project", "roles", "paths", "validation", "testing", "evidence", "budgets", "stop_conditions", "manual_acceptance"}
    ...
    _keys(roles, "$.roles", {"sol", "luna"}, errors)
...
def _contract(document: dict[str, Any], errors: list[str]) -> None:
    required = {"schema_version", "kind", "task", "project_profile", "phase", "writer", "allowed_paths", "test_policy", "luna_mode", "validation", "evidence", "budgets", "stop_conditions", "manual_acceptance"}
